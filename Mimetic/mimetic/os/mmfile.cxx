...
bool MMFile::map()
{
    m_beg = (char*) mmap(0, static_cast<size_t>(m_st.st_size), PROT_READ, MAP_SHARED,m_fd,0);
    if(m_beg > (void *)0)
    {
        m_end = m_beg + m_st.st_size;
        #if HAVE_MADVISE
        madvise(m_beg, static_cast<size_t>(m_st.st_size), MADV_SEQUENTIAL);
        #endif
        return true;
    }
    return false;
...
{
    if(m_beg)
        munmap(m_beg, static_cast<size_t>(m_st.st_size));
    if(m_fd)
        close();
}
